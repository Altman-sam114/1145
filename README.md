...
- Selecting player active sonar sensors overlays their sonar coverage rings on the battlefield; selecting player Carriers or Battleships overlays warm escort radius rings; selecting bound Carrier guard aircraft or HOLD Carriers with bound guard aircraft overlays a guard-radius ring on the anchor Carrier; multi-selection shows multiple rings, while unfinished Sonar Buoys, enemy sensors, enemy high-value naval units, unbound aircraft, HOLD Carriers without bound guard aircraft, and non-selected units do not show these selection-derived coverage visuals.
...
- Tap ground with units selected to move in formation; if the order releases bound Carrier guard aircraft, the success message reports how many left `CV GUARD`.
- Tap a visible enemy with combat units selected to attack; if assigned aircraft leave a Carrier guard wing for that attack, the success message reports the release count.
- Tap `HOLD` with mobile units selected to make them guard their current positions and return if pulled too far away; when a selected Carrier holds, nearby friendly operational Helicopters/Fighters are also assigned to hold as its guard wing with a short `CV GUARD` deck cue, bound to that Carrier for the compact `GW` HUD count, kept on stable nearby guard stations, reported as `CV GUARD` with anchor distance and an anchor Carrier guard-radius ring when those aircraft or the HOLD Carrier with bound aircraft are selected, and biased toward known attackable threats near that Carrier while it remains holding until they receive a new move, attack-move, attack, or ordinary hold order. Successful ordinary hold orders also report how many previously bound aircraft left `CV GUARD` when they are not immediately rebound to a Carrier.
- Tap the bottom HUD `AMOV` button with combat units selected, then tap the map to advance in formation while engaging visible enemies en route; if the attack-move releases bound Carrier guard aircraft, the success message reports how many left `CV GUARD`. During the final Red HQ objective, a known Red HQ gets a short target cue plus HP and approximate nearest-distance details in the selection panel when `AMOV` is armed.
