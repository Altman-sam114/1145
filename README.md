...
- Selecting player active sonar sensors overlays their sonar coverage rings on the battlefield; selecting player Carriers or Battleships overlays warm escort radius rings; multi-selection shows multiple rings, while unfinished Sonar Buoys, enemy sensors, enemy high-value naval units, and non-selected units do not show these selection-derived coverage visuals.
...
- Tap `HOLD` with mobile units selected to make them guard their current positions and return if pulled too far away; when a selected Carrier holds, nearby friendly operational Helicopters/Fighters are also assigned to hold as its guard wing, bound to that Carrier for the compact `GW` HUD count, kept on stable nearby guard stations, and biased toward known attackable threats near that Carrier while it remains holding until they receive a new move, attack-move, attack, or ordinary hold order.
