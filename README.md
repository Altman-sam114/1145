...
- Carriers act as mobile sea-based air platforms that can build Helicopters and Fighters, show short deck-launch cues when aircraft are produced, set air rally points, surface `CV` as the `HELI` / `JET` button source when selected or least busy, report deck production, rally set/unset state, nearby HEL/JET wing readiness with `Wing x/2 OK` or `Wing x/2 Need n`, switch that line to true `Guard wing x/2 OK` or `Guard wing x/2 Need n` status while the Carrier itself is holding and only count nearby HEL/JET still bound to that Carrier's guard anchor, plus nearby escort status with `OK` or `Need n Air/Sea/Ground/Mix` hints in the single-selection panel, and contribute to multi-selection high-value naval escort summaries that also surface gap-type hints when under-escorted; selected player Carriers and Battleships show an escort radius ring for the same nearby-escort range.
...
- Unit selection, select-all army button, hold-position guard orders, Carrier HOLD guard-wing assignment for nearby friendly operational Helicopters/Fighters with lightweight Carrier anchor binding, attack-move advances, move orders, attack orders, health bars, explosions, automatic mechanic repairs, mechanic repair range feedback, and repair sparks.
...
- Tap `HOLD` with mobile units selected to make them guard their current positions and return if pulled too far away; when a selected Carrier holds, nearby friendly operational Helicopters/Fighters are also assigned to hold as its guard wing and bound to that Carrier for the Guard wing HUD count until they receive a new move, attack-move, attack, or ordinary hold order.
