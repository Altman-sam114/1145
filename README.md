...
- Tactical selection panel with current unit/building stats, production state, rally/queue information, Carrier deck Helicopter/Fighter status, Carrier/Battleship nearby escort status with `OK` or `Need n`, multi-selection high-value naval escort summaries, escort radius visuals, selected-player-Mechanic repair range rings and damaged-target counts, damaged-player-mobile-unit nearest Mechanic repair-source hints, Shipyard/Sonar Buoy/Coastal Battery coastal-asset roles plus `Secure Coast` counted/pending/not-counted state, ASW attack versus sonar sensor status, submarine detection state, and multi-unit formation composition.
