...
- 玩家单击未精确命中任何实体时，先在选中作战单位可攻击的敌军中执行 26pt 屏幕尺度辅助命中，再回落到友军选择辅助与地面移动；精确敌军点击仍优先进入既有直接攻击，缩远镜头下点击小型敌机 / 敌车不再误触发移动。
- 辅助候选只收集存活、玩家已知（迷雾与未侦测 Submarine 通过 `isKnownToFaction` 排除）且至少能被一个选中单位 `canAttack(...)` 的敌军，命中半径取 `max(footprint * 0.95, 26pt * cameraScale)`，按 0.5 世界距离桶与 entity ID 的严格总序取最近者。
- 辅助攻击复用 `issueDirectAttackOrder` 共享 helper，仅新增 `TAP CODE` marker 文案覆盖；只改写能攻击该目标的单位并清理互斥命令，混编中不兼容单位保留旧命令。不改变 AI、迷雾、潜艇侦测、经济或胜负逻辑。
- 新增 capture-only `enemy-touch-assist`：两台 Blue AA Truck 与一架 64% HP Red Fighter，precondition 证明点击点严格位于 Fighter 精确 footprint 外、26pt 辅助半径内且 `entity(at:)` 为 nil，再真实调用辅助 helper；GitHub Actions 从二十二次扩展为二十三次独立启动并新增 `simulator-enemy-touch-assist.png`。
- README、核心 flow、flowchart、测试规范和 v5.3 提示词已同步。人工 `project.pbxproj` Team ID 与未跟踪 Unity 报告保持未暂存。

验证结果：

- 按人工要求未运行本地 Xcode build、本地 simulator 或本地玩法探针；本机只运行 `git diff --check`、`git diff --cached --check` 等轻量检查并通过。
- 实现提交：`d94050259a1258f8c2fd360718ad02301076bd8a`，commit subject 为 `v5.3: 加入敌军触屏命中辅助`。
- GitHub Actions run：`30197612826`，attempt `1`，conclusion `success`，job 总耗时约 17 分 33 秒。
- artifact：`desert-frontline-ci-v5.3-main-d94050259a12-run30197612826-attempt1`，artifact ID `8630763291`，缓存于 `/private/tmp/desert-frontline-c-review-30197612826/`，未加密且约 30 MB。
- manifest 记录 `branch=main`、`commitSha=d94050259a1258f8c2fd360718ad02301076bd8a`、`runId=30197612826`、`runAttempt=1`、`version=v5.3`，build、static checks、project lint、simulator launch 均为 success。JUnit 记录 4 项 CI 检查、0 failures、1 skipped；skipped 仅表示当前没有 XCTest target。
- generic iOS device 与 simulator build 日志均包含 `** BUILD SUCCEEDED **`；二十三次 simulator launch 均在截图后 "still running"，二十三张 PNG 均为 1206x2622，App / launch 日志的 crash、fatal error、SIGABRT、watchdog、未捕获异常命中为 0。验收账号确认为 `Altman-sam114`。
- 目视 `simulator-enemy-touch-assist.png`：两台可辨 Blue AA Truck、一架 Red Fighter 与地面投影、两条红色攻击意图线、持久 `ATK TAP JET`、`FOCUS 2 JET 64%` 世界标注与八段目标血条、`2 units selected | FOCUS 2 JET 64%` 摘要、`2 UNITS | ENG 2` 详情面板（`PRIMARY JET x2 HP 134/210 64%`）、完整九动作 TACT 单排（含 `TGT JET 1/1`）与小地图；原图 SHA-256 为 `bef9ac874669bf843bc09dc9c8397afca71b3fa62794861765a751b081f138d9`。
- 代码审查确认：辅助候选过滤尊重迷雾与 `canAttack` 边界、排序满足严格弱序、辅助优先级位于精确命中之后 / 友军辅助之前、攻击执行复用共享 helper。
...
### v5.9 / 红色虚线攻击目标环与橙色双 V

日期：2026-08-09

核心变更：

- 参考百度百科 lemma id `4042982` 对应的 Noble Master Games《沙漠风暴 Desert Stormfront》截图语言，将共享 `showAttackTargetMarker(...)` 从红色实心椭圆 / 四角括号 / 中心点重绘为 footprint-aware 红色虚线椭圆目标环，保留低透明内填充与深色底线以适配沙地、水面和单位模型。
- 目标环上缘内侧新增两枚向环中心下指的琥珀色双 V cue，使用深色背线 + 亮色前线提升对比；`ATK JET`、`ATK 2/3 ART`、`ATK TAP JET` 等既有标签移到环下方。双 V 仅是攻击目标识别反馈，不表示移动方向、弹道、目标优先级或新的命令状态。
- 直接点击、`TGT` 目标循环和敌军触屏辅助继续共享同一 helper；`presentCommandMarker(...)` 的普通淡入 / 淡出 / 移除、persistent capture 行为、footprint 限幅、攻击者筛选、迷雾 / 潜艇侦测和所有玩法状态均未改变。未新增第 25 次 launch，继续复用二十四次既有 workflow 探针。
- `README.md`、`md/flow/flow.md`、`md/flow/flowchart.md`、`md/test/test.md` 和 v5.9 提示词已同步；人工 `project.pbxproj` Team ID 与未跟踪 Unity 分析目录保持未暂存。
...
- `GameScene.swift` 只在 `addStructureBody(for:to:)` 的 `.shipyard` 分支，在既有 dock / crane 内增加两条短内轨、三处低矮龙骨支架、一段窄内侧码头边缘和三个小系泊标记。几何全部直接挂在既有实体 `base`，使用静态 SpriteKit shape 和既有颜色；没有新增状态、helper、动画、随机、每帧创建、效果层或外部素材，也没有改变 `EntityKind.shipyard.footprint`、碰撞、施工、生产、rally、AI、海岸合法性、fog、死亡或 `SKRM` 重开链路。
- `README.md`、`md/flow/flow.md`、`md/flow/flowchart.md`、`md/test/test.md` 与 v5.28 提示词已同步；用户未提交的 `DesertFrontline.xcodeproj/project.pbxproj` Team ID 修改、`md/unity分析/` 和 v5.23–v5.28 未跟踪提示词均保持未触碰。

验证结果：

- 实现提交：`0e5cb6c621d1cd93cfd8224b4ba32d20c07fca2e`，commit subject 为 `v5.28: 造船厂坞区模型细化`；验收时本地 `main`、`origin/main`、GitHub `main` 和 Actions head 均为该 SHA，活动账号为 `Altman-sam114`。
- 首次 run `32310608461` attempt `1` 的 artifact `9386508101` 未被接受：其 `simulator-land-combat.png` 只有 `68332` bytes，目视为白底加顶部黑色胶囊，没有地图、单位或战斗 HUD；同一 artifact 的其他截图正常。对比 v5.27 日志闭环 run `32305507552` 中另一张首发截图也出现过 `67 KB` 白屏，且 v5.28 Swift diff 只涉及 Shipyard 分支，因此判定为单张云端 capture 时序波动，不把它冒充视觉通过。
- 同一 commit 重跑后的准确 Actions run：`32310608461`、attempt `2`、job `96259174161`，conclusion `success`；artifact ID `9387320732`，名称 `desert-frontline-ci-v5.28-main-0e5cb6c621d1-run32310608461-attempt2`，缓存于 `/private/tmp/desert-frontline-c-review-32310608461/attempt2/`。官方未加密 ZIP `/private/tmp/desert-frontline-c-review-32310608461/attempt2/artifact-9387320732.zip` 大小 `32928714` bytes，SHA-256 为 `250f5cbd24b841a1f554145fe1e1cd1d64a66d38e0b900678cac92a921ee41c1`，`unzip -t` 通过并含 40 个文件。
- attempt 2 manifest 与 `main`、`origin/main`、Actions head 完全匹配，记录 `branch=main`、`commitSha=0e5cb6c621d1cd93cfd8224b4ba32d20c07fca2e`、`runId=32310608461`、`runAttempt=2`、`version=v5.28`、`destination=generic/platform=iOS`；static checks、project lint、generic iOS build、simulator launch 全部为 `success`。JUnit 为 4 项 CI 检查、0 failures、1 skipped；唯一 skipped 是当前项目没有 XCTest target。`xcodebuild.log` 含 `** BUILD SUCCEEDED **`，project lint 为 `OK`，`ci-failure-summary.md`、`simulator-launch.log`、`simulator-app.log` 和 `DesertFrontline.xcresult` 均来自同一 attempt。
- simulator launch log 记录 24 次独立启动、24 张截图及截图后 PID 仍存活；app / launch 日志未发现 crash、fatal error、SIGABRT、watchdog、未捕获异常或进程提前退出。重复的 UIKit / SpriteKit focus 与 `getpwuid_r` 系统提示不构成 App 崩溃。
- manifest、解压目录和 ZIP 均核对出同一组 24 张 PNG；全部为 `1206x2622`、`8-bit/color RGBA`、`non-interlaced`，24 个 SHA-256 均唯一，文件大小范围为 `1092072–1613083` bytes。哈希如下：`simulator-carrier-strike.png` `a3a8bcf33c6b90802cc69c8f1b55d0501d81b8e55a050899d6018fae425f456e`；`simulator-coastal-battery.png` `40122676c903ad695650921d412d26a837b4150f9224c4fd90e6598ee8e99e81`；`simulator-combat-ui.png` `d301443d99e10011f5572b6666366665fee8b8d662c6a3eace32a01d195389b9`；`simulator-command-attack-move.png` `ba3c7c448cb7232079e5afa4a7f63f3048062dfef6e12c33c72b018ef7faa8f3`；`simulator-command-attack-target.png` `f35ce3eaa9614e814397b918947dd95fb12281137ac57f06363b5a5c900f1d45`；`simulator-command-move.png` `980ee17e93f58767b3815792f682dcf656db8724cd5902ba1874f6b956fd66d9`；`simulator-damage-state.png` `1bf32f0ec507fc140db6774e3389e35980691da26ef227bb715663c358e7013c`；`simulator-enemy-touch-assist.png` `d3ca9726fd6dbc6288598be9e74ca3a6dd615127b666acb4a9d7ae2e58d435a7`；`simulator-fighter-strike.png` `794aa006555d710a7adb59c71cedd3c173e2cfecc1b93f06b5e7dceba38136a4`；`simulator-helicopter-salvo.png` `e72ffcf3cb182a8438535cbf40e64c93425645f4a380be19893be4717b78bc1e`；`simulator-hud-air.png` `93a20eaa0e9ce3228532126e4a610e2a0e1c17650a5076c611c9d976aefa00fd`；`simulator-hud-build.png` `3d9fa017b345a08aadd15b4f947fa3ef447a55fb091a36f28cbfdfedda580e2c`；`simulator-hud-naval.png` `71c2b4f64f975dd361722ce4cb332ecacdd6e2c84acc848869933788bc4f755b`；`simulator-hud-support.png` `2d01bfd867ac9edf015c57133dcac577c7360d7b477062e19aaacc6dc3006b16`；`simulator-incoming-ui.png` `59f19bb7889ca4017ed61b28788f7937b5498575bc36d13250166461fb8179bc`；`simulator-land-combat.png` `b1df8b608f08c5f83911cb462c2b512783cfd57f8a98c14e82026956376b8ccf`；`simulator-map-terrain.png` `080dcf0b34313b18b7183c565068275024a0dc70a5c900868a2603ce6b0c2899`；`simulator-mobile-aa.png` `a855328023eb1d79071da3b08f6d81b2c3025650aebe297e8b92d5e421f50130`；`simulator-naval-damage.png` `5f2bb0b882fb7332eb6194eb885c80c03db1f25c30ec75f0b97d8b8767d7b7c2`；`simulator-naval-salvo.png` `aea4c357772db20bf50d6415c684b537b7d0508b45f8624f6b132c1f3cd536b7`；`simulator-screenshot.png` `96b2faad0d8005a5c9b7d0a155b7706aa5c40be7247af8a044773f63b028bfb5`；`simulator-selection-cycle.png` `b5ee0584c67ebcd97fd62bbfb9a4a9502c4c2af4b11cb17bdf5595373a4f8264`；`simulator-stop-command.png` `672c0176db9ac0b03eca47ba177228cade0976fdebc8ee6fead633997c61806d`；`simulator-target-cycle.png` `f8ace9a4988067f2b0b83388c1e6b7386b326148ecac0b0d124b41c7a78f4f17`。
- 目视确认 attempt 2 的 `simulator-land-combat.png` 已恢复完整陆战地图、单位、交战线、HUD 和小地图；`simulator-coastal-battery.png` 为主 Shipyard 证据，放大核对可见两条内轨、三处支架、窄内侧码头边缘和三个系泊点，均低对比且留在原 dock / crane 包络内。`simulator-map-terrain.png` 未见 Shipyard 污染浅水、泡沫、道路、地形或小地图；`simulator-naval-salvo.png`、`simulator-carrier-strike.png`、`simulator-naval-damage.png`、`simulator-hud-build.png` 与其余 20 张 PNG 未见海战、战损、HUD、迷雾或非 Shipyard 场景回归。
- 本轮按规则未运行本地 `xcodebuild`、Simulator、`simctl` 或本地玩法探针；本机只运行了 artifact 完整性、PNG 解码 / 哈希、源码边界审阅、`git diff --check` 和云端证据核对。
