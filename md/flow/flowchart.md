...
当前版本视觉增量：实体配置阶段创建旗杆阵营旗标旁的六格竖直耐久塔，为单选 Blue 陆空作战单位预创建由既有 `attackRange` 驱动的低透明等距射程椭圆，为 Carrier 预创建三个固定停机位舰载机轮廓，并为 Submarine 创建低对比度艏艉细节与实体本地 `STEALTH` / `SONAR` / `DETECT` / `CONTACT` cue；HUD 选择面板在写入行文本时使用集中式海空短码与有界单行 fitting，小地图海军/空军 blip 另增加已知朝向与选中交战微齿；这些节点仍沿用现有迷雾、选择、告警和死亡清理链路，所有新增 cue 都不改攻击或生产规则。
...
- v5.17：小地图海军菱形与空军三角增加由已知目标 / 移动意图 / 镜像回退驱动的短朝向齿；选中 Blue 海空单位对合法已知目标显示红色目标齿、存在已知来袭时显示琥珀来袭齿；不新增状态、文字、命令或第 25 次探针。
