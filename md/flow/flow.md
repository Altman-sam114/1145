...
- GitHub Actions 在 `main` push 或手动触发时运行 `ci-results.yml`，生成 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`ci-failure-summary.md` 和 `.xcresult`；simulator launch 通过 `DESERT_CI_CAMERA_FOCUS=coast` 只在云端截图时把镜头聚焦玩家 Shipyard 海岸，普通玩家默认启动镜头不变。
