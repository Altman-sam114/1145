...
- Mechanic 自动维修仍由 `updateRepair(dt:)` 统一执行：寻找同阵营、存活、非自身、受损的最近目标，目标距离小于 95 时按每秒 22 修复并偶发 repair spark；选择面板和维修范围圈只读复用该范围，不改变目标选择、维修量、AI、支援或移动 / 战斗规则。
