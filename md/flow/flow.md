...
5. 初始化流程依次构建地形、绘制地图、生成控制点、生成初始单位/建筑、布局 HUD、刷新迷雾、刷新 HUD；`drawMap()` 用 tile 坐标与 skirmish seed 的确定性散列选择小幅沙地色差并低密度生成沙丘等高线 / 风纹，只读检查 road 四个正交邻格后按真实连通方向绘制道路肩、内层路床和中心标记，为 ridge 一次性生成不同尺寸 / 偏移的岩体落影、亮暗岩面与碎石，为 oil 增加污环 / 裂纹；水格继续只读检查四个正交邻格，在邻陆水格叠加两级浅水菱形（外层浅青 + 内缘亮青渐变）、青白冲洗带、细泡沫线和沿岸线确定性散布的白色泡点，开阔水格保留稀疏方向波纹并按哈希确定性变化波杆长度 / 角度、约半数追加第二条短波杆；`.sand` / `.oil` 陆地格沿同一四方向边表为水邻边加入缩进、低对比湿沙唇与稀疏断续高光，其中 `.sand` 每个 tile 还按 `terrainDetailHash(for:)` 与 `edge.index` 稀疏选择至多一条短平行潮痕和一个小漂砾，`.ridge` 陆地格按哈希为水邻边加入短暗色礁石轮廓与少量碎片，`.road` 不增加陆地侧岸线装饰。水侧与陆地侧都只创建 `mapNode` 子节点，并受既有 `fogLayer` 覆盖；所有这些节点只在地图构建 / 重开时创建，不进入每帧 update，不改变 terrain、移动速度、寻路、建造、海岸、迷雾或小地图语义。
...
Mechanic 实体另持有预创建的 `mechanicRepairEffectNode`，由阵营化外层束、白色核心束和十字目标脉冲组成。`updateRepair(dt:)` 每帧先隐藏所有 Mechanic 维修节点，完成原最近受损友军与 95 范围判断、每秒 22 HP 恢复后，再由 `updateMechanicRepairEffect(...)` 把目标世界坐标转换为考虑实体 `xScale` 的局部路径；无合法目标时保持隐藏。玩家 Mechanic 的实际维修链路可见，敌方链路要求 Mechanic 和目标都通过玩家现有认知过滤，节点属于 Mechanic 子树，不按帧新增或绕开迷雾泄露目标。
...
`addLandUnitBody(...)` 中的 Humvee 使用四轮、前后装甲 / 保险杠、引擎盖、装甲座舱、阵营化风挡、车顶枪座 / 枪管、前灯和天线；Tank 使用分层履带 / 负重轮、车体前装甲、炮塔环、舱盖、阵营化光学件、炮盾、炮管和制退器；Artillery 使用履带 / 负重轮、后部稳定铲、装甲战斗室、炮座 / 炮盾、舱盖、阵营化光学件、分段长炮管和制退器；Mechanic 使用四轮、工程底盘、后部工具舱 / 槽位、驾驶舱、阵营化风挡、警示灯、折叠吊臂 / 吊钩和工具色标。这些 SpriteKit shape 都属于原实体 `base` 子树，随实体移动、镜像、迷雾和销毁，不修改碰撞、数值或武器状态。
...
- 玩家成功普通移动会在每个实际 domain anchor 显示青绿色 `MOVE` 贴地环、箭头与短刻度；成功 AMOV 显示琥珀色双环、十字推进箭头和 `AMOV` 标签；成功直接攻击至少分配一个合法攻击者后，才按已知目标 footprint 显示红色虚线目标环、环上缘内侧的琥珀色双 V 下指 cue 和环下方 `ATK <shortCode>` 标签。三类组合节点都进入 `effectsLayer` 并在约 0.68 秒内淡出移除，不改变 destination、attack target、编队、伤害、AI 或迷雾；直接攻击目标仍先经过 `entity(at:)` 的玩家认知过滤，不能通过反馈泄露隐藏敌军或潜艇。
- 玩家选中且存活的机动单位额外显示只读命令意图线：优先级为已知 `attackTarget`（红虚线）>`attackMoveDestination`（琥珀虚线）>`destination/path`（青绿虚线）>`holdPosition`（淡金虚线）；线挂在实体 `commandIntentNode`，随 `refreshSelection` / `updateHUD` 刷新，取消选中即隐藏。不改命令语义、寻路、伤害、AI 或迷雾；对未知敌军目标不画攻击线。
...
- `updateCombat(dt:)` 会先跳过并清理未完工攻击型结构的攻击目标和计时器；完工结构与移动单位再选择目标、用老兵等级计算有效冷却并调用 `fire(attacker:target:)`。Carrier 对已接受的已知 `.air` 目标在同一 `fire` 视觉分支中显示 `CV INTERCEPT` 甲板脉冲、舰载战斗机拦截飞行、导引光迹和 `showAirMissileImpact(...)` 风格空中命中环；该分支只创建短生命周期 `effectsLayer` 节点，不写回伤害、目标、冷却或迷雾状态。
...
- `updateSelectionInfoPanel(...)` 每帧先把所有既有行恢复为中性浅色，再按同一只读状态源设置颜色：自身 HP 复用 `damageStateShortLabel(...)` 的 DMG / CRIT 阈值，主目标行与 `updateSelectionTargetHealthBar(...)` 复用 55% / 25% 目标 HP 阈值，Weapon ready / Reload 只读 `attackTimer`，合法 incoming 优先用红橙色；pending 建造 / 集结 / 支援 / attack-move、空选择和非战斗摘要不继承上一次颜色。颜色只属于 HUD presentation，不读取未知目标或写回战斗状态。
- 当至少 2 个玩家选中作战单位共享同一合法主目标时，`effectsLayer` 上的 `focusFireMarkerNode` 使用确定性的 FOCUS 层（本地 zPosition 244，全局约 434，高于空军实体最高约 398、低于更高的弹体 / 爆炸效果）在目标显示红色双环 / 十字、`FOCUS n <shortCode> <hp>%` 和八段生命条；空军、海军与陆地目标使用不同的 caption lift，生命段固定下移。直接攻击 marker 使用独立的 ATK 层（本地 zPosition 270，全局约 460），其 `ATK <shortCode>` 文案固定再下移，和 FOCUS 生命段确定性分带；无共用目标或目标不再已知时隐藏。只读刷新，不改攻击分配、伤害或迷雾。
- `refreshIncomingThreatVisuals(for:)` 在每次 HUD 刷新时只扫描一次敌方实体，只接收存活、operational、有伤害、玩家已知、真实 `attackTarget` 指向当前选中玩家实体且 `canAttack` 合法的攻击者，并按目标建立 `incomingThreatsByTargetID` 只读快照。单选面板显示 `INCOMING n <code> Dn` 或在 ready / reload 行追加紧凑来袭摘要，多选标题与最后一行显示 `IN n`；受威胁的选中实体使用预创建双重红橙方向箭头和 `IN n` 标签指向最近攻击者，counter-scale 抵消单位 `xScale=-1`，无威胁时隐藏。该链路不写 `attackTarget`、HP 或计时器，也不统计未知、死亡、未完工或非法攻击者。
- 选中玩家存活且 operational 的 Battleship / Coastal Battery 时，实体 `navalGunRangeNode` 显示基于 `attackRange` 的暖色等距压缩射程椭圆；取消选中隐藏。只读，不改射程、伤害、AI 或迷雾。
- `fire(attacker:target:)` 使用有效伤害结算直接开火；Carrier 只在来源对玩家已知时派发纯视觉 `launchCarrierWing(...)`，以三条错列二次曲线组织三机进场 / 俯冲、阵营色航线、低透明烟迹和机尾亮焰，再从编队前缘释放两枚带弹体 / 尾焰 / 烟迹的反舰弹并在目标端由 `showCarrierStrikeImpact(...)` 组合紧凑近舷水溅射、较小近失回响、舰体闪光与导弹命中环；水面回响的椭圆方向复用 Carrier 到目标的来袭向量，普通节点沿 `showNavalWaterImpact(...)` 生命周期清理，capture-only 才可 persistent。该链路不创建真实舰载机 entity，不改变目标、AI、迷雾或冷却，`target.hp` 仍只在统一 `fire(...)` 位置写一次；直接火力命中玩家潜艇或玩家已知敌方潜艇时，`showAntiSubmarineHit(...)` 组合双层压力环、紧凑浅青水沫 / 小水柱、确定性气泡和 `ASW HIT`，普通节点约 0.6 秒内移除且仍只从既有合法已知门槛调用，不改变伤害、侦测或暴露；击杀发生时为可移动作战单位增加 `killCount` 和 XP，XP 推动 Recruit / Hardened / Veteran / Elite 等级，并更新头顶徽章。
- 潜艇的 `addNavalUnitBody(...)` 在原椭圆舰体 / 指挥塔上叠加低对比度舰体带、艏部声呐穹顶、双鱼雷口、鞍部舱盖、潜望镜、尾鳍、舵和螺旋桨；细节与 `submarineDetailNode` 一起挂在实体子树，沿用实体镜像、父节点迷雾和死亡清理。`refreshSubmarineContactCueVisuals()` 只更新预创建的实体本地环和短标签：选中的玩家潜艇只读 `revealedUntil`、`isCoveredByKnownEnemySonar(...)` 显示 `DETECT` / `SONAR` / `STEALTH`，玩家已知敌方潜艇显示中性的 `CONTACT`；未知敌方或不可见实体隐藏 cue，不改 `revealedUntil`、声呐、目标、伤害或攻击规则。
- `explode` 使用烟尘底环、火核、闪光环和碎片点分层短动画；CI 可 persistent 冻结。击杀结算不变。
- Battleship 的 `addNavalUnitBody(...)` 实体树包含分层装甲带 / 甲板 / 首尾装甲、两座炮塔座圈 / 炮盾 / 双炮管、舰桥与阵营舷窗、桅杆 / 雷达、二级炮和救生艇；Carrier 的同一函数包含深色甲板边缘、蓝灰主甲板、斜角跑道区（中线虚线 + 两端门槛线）、两个停机位圈、右舷舰岛（底座 / 塔楼 / 阵营舷窗 / 天线杆）；均仍使用原 footprint、碰撞、选择、血条、镜像与迷雾节点。`fire(attacker:target:)` 对玩家 Battleship / operational Coastal Battery 或玩家已知 Red 炮位调用纯视觉 `showNavalGunSalvo(...)`，生成两条紧凑平行的阵营强调线 / 暖白亮芯、两枚弹体、双炮口闪光和烟团；普通播放给第二条 lane 增加确定性的 0.045 秒视觉错列，capture-only persistent 播放仍一次创建两条完整 lane；未知 Red 炮位不创建该来源反馈。命中玩家水面舰艇或玩家已知敌方水面舰艇时，Battleship / Coastal Battery 用 `showNavalSalvoImpact(...)` 组合主 / 副水柱和舰体闪光 / 火花；运行时 `fire(...)` 传入攻击者位置，让两处水柱按来袭方向错位并旋转局部椭圆，未传位置的直接 helper 调用保留 entity-id 确定性 fallback。Artillery 保留原单水柱，Submarine 继续只走既有 ASW 反馈。所有普通节点约 0.6 秒内移除，capture-only 才可 persistent；`target.hp` 仍只在原 `fire(...)` 位置写一次，效果不改变伤害、目标、迷雾或暴露。
...
- 小地图显示地形、控制点、可见单位和相机框，点击可跳转相机；实体符号按领域区分为结构方块、陆军圆点、空军三角和海军菱形，潜艇使用低填充空心菱形，Battleship / Carrier 使用更醒目的描边，当前 `selectedIDs` 实体增加高对比外圈；选中且存在同一已知来袭快照的玩家实体再增加红橙告警外圈。敌方符号与来袭攻击者都先经过 `isKnownToFaction(..., observer: .player)`，不会借小地图泄露迷雾单位或未侦测潜艇。
...
- 第二十四次使用 coast focus 与 capture-only `naval-damage` 生成 `simulator-naval-damage.png`：约 29% HP Blue Battleship 与约 46% HP Red Submarine 保持在 Battleship 既有 230pt 声呐范围和玩家可见 tile 内，经 `updateFog(force: true)` / `isKnownToFaction(...)` 真实校验合法已知后，冻结 Battleship 两档战损、Submarine 命中和单选战斗态势；场景不写 `revealedUntil`。二十四次启动均独立解析 PID、截图并检查存活。
