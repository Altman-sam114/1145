...
- GitHub Actions 在 `main` push 或手动触发时运行 `ci-results.yml`，生成 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`ci-failure-summary.md` 和 `.xcresult`；simulator launch 通过 `DESERT_CI_CAMERA_FOCUS=coast` 只在云端截图时把镜头聚焦玩家 Shipyard 海岸，并通过 `DESERT_CI_CAPTURE_MODE=1` 保持 HUD 渲染但暂停经济、AI、战斗和胜负推进，让 runner 延迟期间的截图仍稳定对应初始战场。capture scene 会给初始玩家海军设置确定性移动意图并直接刷新其航迹节点，用于稳定截图验收；普通玩家启动不设置这些变量，默认镜头和实时循环不变。
