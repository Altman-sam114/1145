...
- Red routine assault wave 成功下发后，会先按 `isKnownToFaction(..., observer: .player)` 过滤出玩家已知的 wave 子集；若该子集非空，则记录 `lastEnemyAssaultWaveSummary`，HUD 的 Red 状态行用 `R# F# Wave n Lx/Ax/Nx` 显示最近可感知主攻波宏观构成，并在非零时追加 `CVc`、`Hh`、`Jj`；没有可感知 wave 摘要时仍显示 AI 难度。该摘要只来自已经下发且玩家已知的 wave 单位，不包含目标坐标、隐藏单位位置、隐藏单位组成或额外侦察信息，`SKRM` 重开会清空。

### HUD / 小地图

- HUD 每帧由 `updateHUD()` 汇总金钱、收入、队列、任务、兵力、AI 状态、选择信息和按钮状态。
- AI 状态行默认用 `R# F# AI ...` 显示 Red 兵力、旗点和难度；Red routine 主攻波成功下发且至少一个 wave 单位对玩家已知后，难度位置会替换为最近可感知 wave 子集的短构成摘要，用于提示玩家 Red 已暴露推进部队的混编规模和 CV / HEL / JET 参与情况。
