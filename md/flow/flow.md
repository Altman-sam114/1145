...
- GitHub Actions 在 `main` push 或手动触发时运行 `ci-results.yml`，生成 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`ci-failure-summary.md` 和 `.xcresult`；simulator launch 当前通过 `DESERT_CI_CAMERA_FOCUS=air` 只在云端截图时聚焦中央空战区域，并通过 `DESERT_CI_CAPTURE_MODE=1` 保持 HUD 渲染但暂停经济、AI、战斗和胜负推进。air capture scene 会临时编排 Blue / Red HEL/JET、刷新方向投影，并保留一条 Fighter 导弹烟迹 / 弹体和目标命中环，经 runner 延迟仍可截图；普通玩家启动不设置这些变量，初始单位、默认镜头和实时循环不变。
