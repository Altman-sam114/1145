...
- 玩家成功普通移动会在每个实际 domain anchor 显示青绿色 `MOVE` 贴地环、箭头与短刻度；成功 AMOV 显示琥珀色双环、十字推进箭头和 `AMOV` 标签；成功直接攻击至少分配一个合法攻击者后，才按已知目标 footprint 显示红色 `ATK <shortCode>` 四角框。三类组合节点都进入 `effectsLayer` 并在约 0.68 秒内淡出移除，不改变 destination、attack target、编队、伤害、AI 或迷雾；直接攻击目标仍先经过 `entity(at:)` 的玩家认知过滤，不能通过反馈泄露隐藏敌军或潜艇。
...
- 当至少 2 个玩家选中作战单位共享同一合法主目标时，`effectsLayer` 上的 `focusFireMarkerNode` 在目标显示红色双环 / 十字、`FOCUS n <shortCode> <hp>%` 和八段生命条；无共用目标或目标不再已知时隐藏。只读刷新，不改攻击分配、伤害或迷雾。
...
- Fighter、SAM Site、AA Truck 的 `showProjectile(...)` 分支使用宽低透明度烟迹、阵营弹道色高亮线和沿弹道移动的发光弹体；命中玩家空军或玩家已知敌方空军时显示双环与六向火花。普通弹道和命中节点分别在约 0.38 / 0.42 秒内移除，隐藏敌方空军不通过反馈泄露位置，也不改变伤害、射速、目标选择或防空规则。
...
- `layoutHUD()` 在横屏使用固定单排命令区，五个 `HudPage` 页签常驻，当前页最多 7 个动作；紧凑尺寸下页签宽 44、动作按可用宽度在 40...76 之间稳定布局，命令区高度由旧双排约 114 降为 54，所有 24 个旧动作仍且只出现于一个页面。`SKRM` 重开恢复 `TACT`；CI capture 可用 `DESERT_CI_HUD_PAGE` 只覆盖初始显示页，不改变普通启动默认值。
...
- GitHub Actions 在 `main` push 或手动触发时运行 `ci-results.yml`，生成 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`ci-failure-summary.md` 和 `.xcresult`；前八次 simulator launch 使用 air focus，第九次 land focus 生成 `simulator-land-combat.png`，第十次 coast focus 生成 `simulator-map-terrain.png`，第十一次使用 land focus 与 capture-only `DESERT_CI_COMMAND_MARKER=combat-ui` 生成 `simulator-combat-ui.png`，第十二次使用 land focus 与 `incoming-ui` 生成 `simulator-incoming-ui.png`，第十三次使用 coast focus 与 `naval-salvo` 生成 `simulator-naval-salvo.png`，第十四次使用 coast focus 与 `coastal-battery` 生成 `simulator-coastal-battery.png`，第十五次使用 coast focus 与 `carrier-strike` 生成 `simulator-carrier-strike.png`。combat-ui 只在 CI 中把四个玩家作战单位绑定到约 43% HP 的同一敌方 Tank 并冻结一个 reload；incoming-ui 只在 CI 中把四个已知 Red 攻击者绑定到两个选中 Blue 目标、把一个 Blue Tank 冻结在约 31% HP，并显示敌方炮线；naval-salvo 只在 CI 中让单选 Blue Battleship 锁定约 47% HP Red Battleship；coastal-battery 只在 CI 中让单选 Blue Coastal Battery 锁定约 54% HP Red Battleship，并冻结双发齐射、双炮后坐、炮床冲击 / 岸边尘浪、主 / 副水柱和舰体命中；carrier-strike 只在 CI 中让单选 Blue Carrier 锁定约 42% HP Red Battleship，并冻结三机错列航线、双反舰弹、舰体命中和水面反馈。普通 App 不进入这些写入分支。十五次启动全部使用 `DESERT_CI_CAPTURE_MODE=1` 暂停经济、AI、战斗和胜负推进并独立解析 PID / 检查存活；air、land、coast 既有冻结证据保持，普通玩家启动的初始单位、镜头和实时循环不变。
