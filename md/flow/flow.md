...
`addLandUnitBody(...)` 中的 Tank 在原 gameplay footprint 与朝向语义内使用分层履带 / 负重轮、车体前装甲、炮塔环、舱盖、阵营化光学件、炮盾、炮管和制退器；Artillery 使用履带 / 负重轮、后部稳定铲、装甲战斗室、炮座 / 炮盾、舱盖、阵营化光学件、分段长炮管和制退器。这些 SpriteKit shape 都属于原实体 `base` 子树，随实体移动、镜像、迷雾和销毁，不修改碰撞、数值或武器状态。
...
- GitHub Actions 在 `main` push 或手动触发时运行 `ci-results.yml`，生成 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`ci-failure-summary.md` 和 `.xcresult`；前八次 simulator launch 通过 `DESERT_CI_CAMERA_FOCUS=air` 聚焦中央空战区域，第九次通过 `DESERT_CI_CAMERA_FOCUS=land` 聚焦确定性双方 Tank / Artillery 对峙样例，全部使用 `DESERT_CI_CAPTURE_MODE=1` 保持 HUD 渲染但暂停经济、AI、战斗和胜负推进。air capture scene 会临时编排 Blue / Red HEL/JET、方向投影、导弹与命中反馈；land capture scene 只在 CI 内编排 Blue / Red Tank / Artillery、persistent 炮口焰 / 炮线和爆炸样本，生成 `simulator-land-combat.png`。五页 HUD、三类 command marker 与 land capture 每次都独立解析 PID 并检查存活；普通玩家启动不设置这些变量，初始单位、默认镜头和实时循环不变。
