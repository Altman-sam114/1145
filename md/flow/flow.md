...
单位和建筑的静态定义来源：名称、短码、域、成本、建造时间、HP、速度、射程、伤害、冷却、视野、占地、生产来源、可生产项、可攻击目标。当前建筑包含 HQ、War Factory、Airfield、Radar Outpost、Guard Tower、Shipyard 和 Oil Derrick；Radar Outpost 是普通陆地结构，完工后提供较大静态视野并可作为 `SCAN` 资产；Guard Tower 是普通陆地防御结构，完工后自动攻击可见 land / air / structure 目标，不攻击 naval / submarine，也不提供声呐。
...
- HQ、已占领油井、已占领旗点提供收入。
- 建筑覆盖由己方已完工结构和己方已占领旗点提供；旗点覆盖范围小于结构覆盖范围。玩家建造仍需要目标位置可见，Shipyard 仍需要海岸，Oil Derrick 仍需要油田，建筑不能压住旗点中心。
...
- 玩家可见性由单位、已完工建筑有效视野、控制点视野和支援侦察共同决定；Radar Outpost 完工后提供较大静态侦察范围，Guard Tower 完工后提供普通防御建筑视野，Veteran 和 Elite 单位会在基础视野上获得轻量加成。
...
- AI 会赚钱、补生产建筑、Radar Outpost 和 Guard Tower、生产混合陆海空单位、占油/旗、重建、使用支援技能、基地被打时拉防守单位。
- AI 常规生产按难度 build pattern 轮转；每个生产槽从 `aiBuildCursor` 开始扫描当前有 operational 生产来源且资金足够的单位，跳过暂时缺 Airfield / Shipyard / Carrier 或资金不足的项，成功后仍通过 `queueBuild` / `productionSource` 排队，未完工建筑不能生产。
...
- AI 进攻使用 attack-move 波次，目标包括旗点、油田、生产建筑、防御结构和 HQ。
...
- 建筑科技层：科技中心、防御塔、SAM、岸防炮、升级解锁和 Radar Outpost 后续平衡。
