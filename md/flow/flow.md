...
人工目标 / Agent A 提示词
  ...
  -> 通过则确认最新 main；失败则退回 Agent B 在 main 追加修复 commit
```
...
- 玩家可见性由单位/建筑有效视野、控制点视野和支援侦察共同决定；Veteran 和 Elite 单位会在基础视野上获得轻量加成。
...
- 角色召唤：`agenta` / `a:` / `A:` 召唤 Agent A，`agentb` / `b:` / `B:` 召唤 Agent B，`agentc` / `c:` / `C:` 召唤 Agent C；无前缀时按普通 Codex 任务处理。
