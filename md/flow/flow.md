...
- 当至少 2 个玩家选中机动单位共享同一存活且玩家已知 `attackTarget` 时，`effectsLayer` 上的 `focusFireMarkerNode` 在该目标显示红色双环 / 十字 / `FOCUS n` 集火标；无共用目标时隐藏。只读刷新，不改攻击分配、伤害或迷雾。
- 同一共用集火目标也会在多选信息面板显示只读 `FOCUS n Tgt <shortCode>` 摘要，与战场标一致。
...
- GitHub Actions 在 `main` push 或手动触发时运行 `ci-results.yml`，生成 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`ci-failure-summary.md` 和 `.xcresult`；前八次 simulator launch 通过 `DESERT_CI_CAMERA_FOCUS=air` 聚焦中央空战区域，第九次通过 `DESERT_CI_CAMERA_FOCUS=land` 聚焦确定性双方 Mechanic / Humvee / Tank / Artillery 对峙样例，第十次通过 `DESERT_CI_CAMERA_FOCUS=coast` 聚焦沙地 / 道路 / 山脊 / 油田 / 海岸 / 水面与玩家海军航迹，分别生成 `simulator-land-combat.png` 和 `simulator-map-terrain.png`。十次启动全部使用 `DESERT_CI_CAPTURE_MODE=1` 保持 HUD 渲染但暂停经济、AI、战斗和胜负推进；air capture scene 会临时编排 Blue / Red HEL/JET、方向投影、导弹与命中反馈，land capture scene 只在 CI 内编排地面单位、维修链路和持久战斗样本，coast/default capture scene 复用既有海军方向航迹与水面命中样本。五页 HUD、三类 command marker、land capture 与 coast capture 每次都独立解析 PID 并检查存活；普通玩家启动不设置这些变量，初始单位、默认镜头和实时循环不变。
