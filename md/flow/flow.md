...
- `fire(attacker:target:)` 使用有效伤害结算直接开火；Carrier 只在来源对玩家已知时派发纯视觉 `launchCarrierWing(...)`，以三条错列二次曲线组织三机进场 / 俯冲、阵营色航线、低透明烟迹和机尾亮焰，再从编队前缘释放两枚带弹体 / 尾焰 / 烟迹的反舰弹并在目标端组合舰体闪光与导弹命中环；普通节点约 1 秒内移除，capture-only 才可 persistent。该链路不创建真实舰载机 entity，不改变目标、AI、迷雾或冷却，`target.hp` 仍只在统一 `fire(...)` 位置写一次；直接火力命中玩家潜艇或玩家已知敌方潜艇时，`showAntiSubmarineHit(...)` 组合双层压力环、紧凑浅青水沫 / 小水柱、确定性气泡和 `ASW HIT`，普通节点约 0.6 秒内移除且仍只从既有合法已知门槛调用，不改变伤害、侦测或暴露；击杀发生时为可移动作战单位增加 `killCount` 和 XP，XP 推动 Recruit / Hardened / Veteran / Elite 等级，并更新头顶徽章。
