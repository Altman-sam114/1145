...
`addLandUnitBody(...)` 中的 Humvee 使用四轮、前后装甲 / 保险杠、引擎盖、装甲座舱、阵营化风挡、车顶枪座 / 枪管、前灯和天线；Tank 使用分层履带 / 负重轮、车体前装甲、炮塔环、舱盖、阵营化光学件、炮盾、炮管和制退器；Artillery 使用履带 / 负重轮、后部稳定铲、装甲战斗室、炮座 / 炮盾、舱盖、阵营化光学件、分段长炮管和制退器。这些 SpriteKit shape 都属于原实体 `base` 子树，随实体移动、镜像、迷雾和销毁，不修改碰撞、数值或武器状态。
