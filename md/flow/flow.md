...
自动任务阶段：占油、夺旗、建立海岸据点、混合军种、摧毁 Red 生产、摧毁 Red HQ。建立海岸据点阶段由 `coastalAssetCount(for:)` 统计己方存活且已完工的 Shipyard、Sonar Buoy 和 Coastal Battery，未完工建筑不计入；该阶段首次完成时 `missionReward(for:)` 通过 `changeMoney(for: .player, by: 600)` 发放一次性资源奖励，依赖 `completedMissionStages` 防止重复发放。
