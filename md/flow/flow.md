...
- GitHub Actions 在 `main` push 或手动触发时运行 `ci-results.yml`，生成 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`ci-failure-summary.md` 和 `.xcresult`；前八次 simulator launch 使用 air focus，第九次 land focus 生成 `simulator-land-combat.png`，第十次 coast focus 生成 `simulator-map-terrain.png`，第十一次使用 land focus 与 capture-only `DESERT_CI_COMMAND_MARKER=combat-ui` 生成 `simulator-combat-ui.png`，第十二次使用 land focus 与 `incoming-ui` 生成 `simulator-incoming-ui.png`，第十三次使用 coast focus 与 `naval-salvo` 生成 `simulator-naval-salvo.png`，第十四次使用 coast focus 与 `coastal-battery` 生成 `simulator-coastal-battery.png`。combat-ui 只在 CI 中把四个玩家作战单位绑定到约 43% HP 的同一敌方 Tank 并冻结一个 reload；incoming-ui 只在 CI 中把四个已知 Red 攻击者绑定到两个选中 Blue 目标、把一个 Blue Tank 冻结在约 31% HP，并显示敌方炮线；naval-salvo 只在 CI 中让单选 Blue Battleship 锁定约 47% HP Red Battleship；coastal-battery 只在 CI 中让单选 Blue Coastal Battery 锁定约 54% HP Red Battleship，并冻结双发齐射、双炮后坐、炮床冲击 / 岸边尘浪、主 / 副水柱和舰体命中，用于验证岸防模型、单选目标 HP / 距离 / reload、射程圈、海岸地貌与单排命令条。普通 App 不进入这些写入分支。十四次启动全部使用 `DESERT_CI_CAPTURE_MODE=1` 暂停经济、AI、战斗和胜负推进并独立解析 PID / 检查存活；air、land、coast 既有冻结证据保持，普通玩家启动的初始单位、镜头和实时循环不变。
