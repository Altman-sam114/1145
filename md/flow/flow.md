...
- GitHub Actions 在 `main` push 或手动触发时运行 `ci-results.yml`，生成 `ci-artifact-manifest.json`、`junit.xml`、`xcodebuild.log`、`ci-failure-summary.md` 和 `.xcresult`；前八次 simulator launch 使用 air focus，第九次 land focus 生成 `simulator-land-combat.png`，第十次 coast focus 生成 `simulator-map-terrain.png`，第十一次至第十七次依次生成 combat-ui、incoming-ui、naval-salvo、coastal-battery、carrier-strike、mobile-aa 与 fighter-strike 专用截图，第十八次使用 land focus 与 `helicopter-salvo` 生成 `simulator-helicopter-salvo.png`。helicopter-salvo 只在 CI 中让单选 Blue Helicopter 锁定约 45% HP Red Tank，并冻结四条短曲线、四枚弹体 / 尾焰、双 pod 闪光、白色烟点、多点命中与伤害飘字；其余专用 capture 继续保持既有编排。普通 App 不进入这些写入分支。十八次启动全部使用 `DESERT_CI_CAPTURE_MODE=1` 暂停经济、AI、战斗和胜负推进并独立解析 PID / 检查存活；air、land、coast 既有冻结证据保持，普通玩家启动的初始单位、镜头和实时循环不变。
