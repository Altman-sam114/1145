...
- 选择信息来自 `singleSelectionInfo(...)` 或 `groupSelectionInfo(...)`，会显示有效攻击/视野、active sonar range、潜艇 stealth / temporary detected 状态、老兵等级、XP、击杀数、多选 sonar asset 数量 / 最大范围和多选等级分布；这些 HUD 文案复用 `EntityKind.hasSonar` 与 `sonarRange(for:)`，不改变声呐检测、潜艇暴露或迷雾合法性。
