...
Agent C 负责验收 Agent B 的结果、维护核心逻辑文档，并在最终通过后按版本号自动提交本轮版本。
...
4. 如验收不通过，必须明确列出阻塞问题、建议修复路径和需要退回 Agent B 的事项；不提交 git。
5. 如验收通过，基于已完成实现更新 `md/flow/flow.md` 和 `md/flow/flowchart.md`。
6. 形成正式版本或重要历史事项时更新 `update_log.md`，记录版本号、核心变更、关键文件、验证结果和遗留事项。
7. 通过验收后自动创建 git commit。提交必须按版本号管理，commit subject 使用 `版本号: 简要说明`，例如 `v1.0: 实现老兵经验系统`。
8. commit body 简要概括本版本做了什么、主要影响范围、验证命令和结果；不要写流水账。
9. 输出通过/不通过、问题清单、已更新文档、验证复核、git 提交哈希、版本工作内容概括和建议下一步。
